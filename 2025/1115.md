...
- 주문/상품이 없어도 모든 카테고리 한 줄씩은 결과에 나와야 함 -> 그래서 기준 테이블은 CATEGORY -> 따라서 LEFT JOIN 구조가 자연스러움

#### LEFT JOIN을 사용해야 하는 이유

- INNER JOIN 을 쓰면 상품이 없는 카테고리, 판매된 적 없는 상품이 있는 카테고리 → 결과에서 완전히 사라짐
- - LEFT JOIN 을 쓰면 기준 테이블인 CATEGORY 는 무조건 유지, 매칭되는 데이터가 없으면 NULL 로 채워짐 → 합산 시 COALESCE 로 처리 가능
